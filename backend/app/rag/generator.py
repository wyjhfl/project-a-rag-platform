...
class ExtractiveGenerator:
        ...
        for chunk in ranked_chunks:
            for sentence in self._split_sentences(chunk.content):
                ...
                if any(marker in sentence for marker in ["处理步骤", "排查步骤", "建议", "检查"]):
                    score += 0.6
                if any(marker in question for marker in ["是否", "可以", "能不能", "还能", "继续"]):
                    if any(marker in sentence for marker in ["禁止", "不建议", "必须", "停机", "人工"]):
                        score += 2.0
                if any(marker in question for marker in ["欠压", "UV-1", "供电"]):
                    if any(marker in sentence for marker in ["UV-1", "欠压", "输入电源", "接触器"]):
                        score += 2.0
    ...
    def _build_conclusion(self, question: str, chunks: list[DocumentChunk]) -> str:
        ...
        if "同一个" in question or "是不是" in question:
            if "UPS" in question.upper():
                return "根据已检索资料，这不是 UPS 电池高风险问题；以下依据对应当前命中的设备资料："
            return "根据已检索资料，这不是同一个故障场景；以下依据对应当前命中的设备资料："
